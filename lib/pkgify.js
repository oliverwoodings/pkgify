...
var getPackageMap = _.once(function (relativeTo, pkgs) {
  ...
  log("Generating pkgify map relative to", relativeTo, pkgs);
...
function transform(args, opts, cb) {
  ...
  if (oldRequire[0] !== ".") {
    ...
    if (pkg) {
      ...
      // replace single backslashes with double backslashes for file paths in windows
      newRequire = newRequire.replace(/\\/gi, "/");
...
// @note just leaving this here for consideration...
function transformFrom(content, opts, cb) {
  opts.config.relativeTo = opts.config.relativeTo || process.cwd();
  verbose = opts.config.verbose;

  // Only look at non-local from statements
  var froms = content.match(reFrom)
  if (froms.length) {
    var pathToRelative = path.relative(path.dirname(opts.file), opts.config.relativeTo);

    for (var index = 0; index < froms.length; index++) {
      var oldImport = froms[index].replace(reFrom, "$1");
      log("Processing", oldImport, "in", opts.file);
      var pkg = findPackage(opts.config, oldImport);
      if (pkg) {
        var newImport = "." + path.sep + path.join(pathToRelative, oldImport.replace(pkg.name, pkg.src));

        // replace single backslashes with double backslashes for file paths in windows
        newImport = newImport.replace(/\\/gi, "/");
        log("Found pkg for", oldImport, ":", pkg.name);
        log("Mapping to", newImport);

        newImport = froms[index].replace(oldImport, newImport);
        content = content.replace(froms[index], newImport);
      }
    }
